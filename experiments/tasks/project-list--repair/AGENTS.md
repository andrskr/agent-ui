...
After you build the task UI, run `vp run repair`. Read the complete diagnostics. Fix the source and
run `vp run repair` again until it passes. Use this same agent session. Run it again after any later
source change. Do not change the check configuration, dependencies, or repair tools. A passing build
