...
three have frozen reports from earlier inputs. Support dashboard and Documentation reader have Opus
4.8 and Sonnet 5 reports after the dependency and instruction update. Their report folders include
screenshots and browser check notes. Earlier run evidence remains in the ledger. The other seven
scenarios have prepared inputs only. Preparation does not start a model or create a ledger record.
...
these limits. Command palette is the next planned scenario. The user can select any prepared
