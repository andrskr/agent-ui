...
fn price(request: &Request) -> Option<f64> {
    ...
    let (mut input, mut output, long) = match model {
        "claude-fable-5" | "claude-fable-5-1" => (10.0, 50.0, false),
        "claude-sonnet-5" => (2.0, 10.0, false),
        "claude-opus-5" => (5.0, 25.0, false),
    ...
    Some(
        (new as f64 * input
            + usage.cached_input_tokens as f64 * input * cache_multiplier
...
mod tests {
    ...
    fn cache_durations_and_context_thresholds_match_codexbar() {
        ...
        assert!((price(&row("claude-sonnet-4-5", 200_001)).unwrap() - 0.633003).abs() < 1e-10);
        ...
        for model in ["claude-unknown", "other/claude-sonnet-4-6"] {
